<?php
/**
 * EUC (Electric Unicycle) Advantages Calculator.
 *
 * Calculates spec-based advantages for EUC comparisons.
...
/**
 * EUC advantage calculator.
 */
class EucAdvantages extends AdvantageCalculatorBase {
	 ...
	 * @var string
	 */
	private string $product_type = 'euc';
	...
	/**
	 * Tire type ranking (best first).
	 */
	private const TIRE_TYPE_RANKING = [
		'pneumatic'  => 1,
		'air'        => 1,
		'inflatable' => 1,
		'tubeless'   => 1,
	...
	/**
	 * Battery brand quality ranking (best first).
	 */
	...
	/**
	 * Get the spec wrapper key for EUCs.
	 *
	 * @return string
	 */
	...
	public function calculate_single( array $product, string $geo = 'US' ): ?array {
		...
		if ( $current_price && $current_price > 0 ) {
			$bracket     = PriceBracketConfig::get_bracket( (float) $current_price, 'euc' );
			$use_bracket = true;
		} else {
			$fallback_info = [
				'reason'  => 'no_regional_price',
				'message' => "No {$geo} pricing available. Comparing against all EUCs.",
			];
		}
		...
		if ( $use_bracket && count( $comparison_set ) < PriceBracketConfig::MIN_BRACKET_SIZE ) {
			...
			$fallback_info = [
				...
				'message' => 'Only ' . count( $comparison_set ) . ' products in bracket. Comparing against all EUCs.',
			];
	...
	private function get_head_to_head_specs(): array {
		return [
			...
			// Features.
			[
				'key'           => 'motor.hollow_motor',
				'label'         => 'Hollow Motor',
				'higher_better' => true,
				'format'        => 'boolean',
			],
			[
				'key'           => 'battery.dual_charging',
				'label'         => 'Dual Charging',
				'higher_better' => true,
				'format'        => 'boolean',
			],
			[
				'key'           => 'safety.lift_sensor',
				'label'         => 'Lift Sensor',
				'higher_better' => true,
				'format'        => 'boolean',
			],
	...
	private function get_single_analysis_specs(): array {
		return [
			...
			[
				'key'           => 'value_metrics.price_per_mile_range',
				'label'         => 'Range Value',
				'unit'          => '$/mi',
				'higher_better' => false,
				'is_value'      => true,
	...
	private function add_category_winner_advantages( array $products, array &$advantages ): void {
		$score_categories = [
			...
			'battery_range'     => 'Best battery & range',
	...
	private function get_multi_comparison_summary( string $key, array $specs ): string {
		...
		switch ( $key ) {
			...
			case 'ride_quality':
				$wheel_size = $this->get_typed_spec_value( $specs, 'wheel.tire_size' );
				$susp_travel = $this->get_typed_spec_value( $specs, 'suspension.travel' );

				if ( $wheel_size && is_numeric( $wheel_size ) ) {
					$details[] = $wheel_size . '" wheel';
				}
				if ( $susp_travel && is_numeric( $susp_travel ) ) {
					$details[] = $susp_travel . 'mm travel';
				}
				break;

			case 'safety':
				$tiltback = $this->get_typed_spec_value( $specs, 'safety.tiltback_speed' );
				$ip       = $this->get_typed_spec_value( $specs, 'safety.ip_rating' );

				if ( $tiltback && is_numeric( $tiltback ) ) {
					$details[] = $tiltback . ' mph tiltback';
				}
				if ( $ip && strtolower( (string) $ip ) !== 'unknown' ) {
					$details[] = strtoupper( (string) $ip );
				}
				break;
			...
			case 'features':
				$count = $this->count_euc_features( $specs );
				if ( $count > 0 ) {
	...
	private function get_single_comparison_set( ?array $bracket, string $geo, bool $use_bracket ): array {
		...
		$all_products = $cache->get_all( 'Electric Unicycle' );
	 ...
	 * Analyze IP rating.
	 *
	 * EUCs are often ridden in varied weather. IPX5+ = strength, IPX4 = neutral, IPX3-/none = weakness.
	 *
	 * @param array $product Target product.
	 * @return array|null Result.
	 */
	...
	private function get_ranking_map( string $ranking_type ): array {
		return match ( $ranking_type ) {
			'tire_type'     => self::TIRE_TYPE_RANKING,
	 ...
	 */
	private function count_euc_features( array $specs ): int {
		$count = 0;

		// Features stored as ACF checkbox array.
		...
		// Safety booleans.
		if ( $this->to_boolean( $this->get_typed_spec_value( $specs, 'safety.lift_sensor' ) ) ) {
			$count++;
		}
		if ( $this->to_boolean( $this->get_typed_spec_value( $specs, 'motor.hollow_motor' ) ) ) {
			$count++;
		}
		if ( $this->to_boolean( $this->get_typed_spec_value( $specs, 'battery.dual_charging' ) ) ) {
			$count++;
		}
	...
	private function get_custom_spec_labels( string $key ): ?array {
		return match ( $key ) {
			...
			'manufacturer_range' => [
				...
				'strong'    => 'Good range',
				'worst'     => 'Worst range',
				'very_weak' => 'Very short range',
				'weak'      => 'Short range',
			],
			'motor.torque' => [
				'best'      => 'Most torque',
				'excellent' => 'Very high torque',
				'strong'    => 'Good torque',
				'worst'     => 'Least torque',
			...
			'wheel.tire_size' => [
				'best'      => 'Largest wheel',
				'excellent' => 'Very large wheel',
				'strong'    => 'Large wheel',
				'worst'     => 'Smallest wheel',
				'very_weak' => 'Very small wheel',
				'weak'      => 'Small wheel',
			],
			'suspension.travel' => [
				'best'      => 'Most suspension travel',
				'excellent' => 'Excellent suspension travel',
				'strong'    => 'Good suspension travel',
				'worst'     => 'Least suspension travel',
				'very_weak' => 'Very little suspension travel',
				'weak'      => 'Limited suspension travel',
			],
			default => null,
		};
	}
	...
	private function get_score_tier_label( string $key, float $diff, bool $is_positive ): string {
		...
		$labels = match ( $key ) {
			...
			'safety' => [
				'excellent'  => 'Excellent safety features',
				'great'      => 'Great safety features',
				'above_avg'  => 'Good safety features',
				'very_weak'  => 'Very limited safety features',
				'weak'       => 'Limited safety features',
				'below_avg'  => 'Below-average safety',
			],
			default => [],
		};
	...
	private function get_score_details( string $key, array $specs, bool $is_advantage ): string {
		return match ( $key ) {
			...
			'safety'        => $this->get_safety_details( $specs, $is_advantage ),
			default         => '',
		};
	}
	...
	private function get_ride_quality_details( array $specs, bool $is_advantage ): string {
		...
		$wheel_size  = $this->get_typed_spec_value( $specs, 'wheel.tire_size' );
		$susp_travel = $this->get_typed_spec_value( $specs, 'suspension.travel' );
		$tire_type   = $this->to_string( $this->get_typed_spec_value( $specs, 'wheel.tire_type' ) );

		if ( $wheel_size && is_numeric( $wheel_size ) ) {
			$details[] = $wheel_size . '" wheel';
		}
		if ( $tire_type ) {
			$details[] = strtolower( $tire_type ) . ' tire';
		}
		if ( $susp_travel && is_numeric( $susp_travel ) ) {
			$details[] = $susp_travel . 'mm suspension';
		}
	...
	/**
	 * Get descriptive details for safety score.
	 *
	 * @param array $specs        Product specs.
	 * @param bool  $is_advantage Whether this is a strength.
	 * @return string Details text.
	 */
	private function get_safety_details( array $specs, bool $is_advantage ): string {
		$details = [];
