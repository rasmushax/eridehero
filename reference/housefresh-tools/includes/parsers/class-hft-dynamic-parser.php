...
/**
 * Dynamic parser that uses database configurations
 */
class HFT_Dynamic_Parser extends HFT_Base_Parser {
    ...
    public function parse(string $url_or_identifier, array $link_meta = []): array {
        ...
        $custom_results = $this->applyCustomRules();
     ...
     */
    private function applyCustomRules(): array {
        $results = [];
        
        foreach ($this->scraper->rules as $rule) {
            if (!$rule->is_active || empty($rule->xpath_selector)) {
                continue;
            }
            
            // Extract value
            $value = $this->extractor->extract($rule->xpath_selector, $rule->attribute);
            ...
            switch ($rule->field_type) {
                case 'price':
                    $results['price'] = $value ? (float)$value : null;
                    break;
